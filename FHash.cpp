// FHash.cpp : Defines the entry point for the console application.
//
...
HMODULE GetProcAddressHash(_In_ DWORD dwModuleFunctionHash, _In_ const char* fName)
{
	PPEB PebAddress;
	...
	while (pDataTableEntry->DllBase != NULL)
	{
		...
		for (i = 0; i < dwNumFunctions; i++)
		{
			...
			log << pFunctionName << " - " << "0x" << std::hex << dwFunctionHash << "\n";
...
int main(int argc, const char* argv[])
{
	if (argc >= 1) {
		...
		{
			LoadLibraryA(argv[2 + i]);
		}
		GetProcAddressHash(0, argv[1]);
	}
	else {
		printf("Usage:\n");
		printf("FHash.exe < output.txt > [ library1.dll, library2.dll, ... ]\n");
	}
